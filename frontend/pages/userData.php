    ...
    public function load()
    {
        ...
        if($userInfo = $user->getUserInfo($user->userId))
        {
            ...
            if(strlen($userInfo['birthDate'])>0){
                $this->templateData['userData'][$counter] = $this->addUserDataElement(Webtext::getText('birthdate', 'Дата народження:', true), 'name', date(DateManager::convert2phpDateFormat(Context::SiteSettings()->getDateFormat()), strtotime($userInfo['birthDate'])));
                $counter++;
            }
