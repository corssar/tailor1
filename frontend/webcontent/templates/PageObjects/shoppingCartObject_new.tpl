...
{literal}
<script>
    var itemsForPopUp = {/literal}JSON.parse('{$itemsForPopUp}'){literal};
    var listPopUpTitle = '{/literal}{webtext keyword='basketListTitle' value='“Ó‚‡˚' remark='Shopping bag page'}{literal}';
</script>
{/literal}
<div id="tinyBask"{* onclick="Basket.show();"*} class="shopping-cart-box">
    <div class="btn-bask {if $productsAmount} pointer{/if}">
            ...
            <span class="text-cleaner">
                ...
                <span>
                    {if $productsAmount}
                        ...
                        <span class="text-el">{webtext keyword="shopping-cart-product_count" value="ÚÓ‚‡ " remark=""}</span>
                        <span class="divider text-el">{webtext keyword="shopping-cart-product_icon" value=" ï " remark=""}</span>
                        <span class="d_i-b">
                        <span class="text-el">180.70</span>
                        <span class="text-el">&nbsp;<span
                                    class="curr">{webtext keyword="shopping-cart-product_symbol" value="Û·" remark=""}</span></span>
                    </span>

                                                {else}

                        <span class="text-el">{webtext keyword="shopping-cart-is_empty" value=" ÓÁËÌ‡ ÔÛÒÚ‡" remark=""}</span>
                    {/if}
                </span>
            </span>
        </button>
        <div id="basketListPopUp" style="height: auto;" class="shopping-cart-popup displayNone shadow">
            <div class="popup-arrow"></div>
            <div class="shopping-cart-content">
                <div class="shopping-cart-header">
                    {webtext keyword="shopping-cart-new-product-added-ru" value="“Œ¬¿– ƒŒ¡¿¬À≈Õ" remark="Shopping cart popup header text"}
                </div>
                <img class="shopping-cart-product-photo" src="#" alt="">
                ...
                <div class="shopping-cart-button">
                    <a class="primary-button shadow" href="{$shoppingbagUrl}">
                        {webtext keyword="shopping-cart-proceed-to-checkout-order" value="Œ‘Œ–ÃÀ≈Õ»≈ «¿ ¿«¿" remark="Shopping cart popup checkout button text"}
                    </a>
                </div>
            </div>
        ...
        <div id="basketPopUp" class="shopping-cart-popup displayNone shadow">
            <div class="popup-arrow"></div>
            <div class="shopping-cart-content">
                <div class="shopping-cart-header">
                    {webtext keyword="shopping-cart-new-product-added-ru" value="“Œ¬¿– ƒŒ¡¿¬À≈Õ" remark="Shopping cart popup header text"}
...
{if $basket.itemsCount > 0}
    <div id="popup-shoppingbag-table" class="hidden drop-style">
        <table class="shopping-bag-items">
            <button class="icon_times_drop" type="button"
                    style="margin-top: 0px; margin-left: 0px; position: relative;">
                <svg height="10" version="1.1" width="10" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <path style="" fill="#ffffff" stroke="none"
                          ...
                          transform="matrix(0.5402,0,0,0.5402,-3.2399,-3.4838)" stroke-width="1.8513333333333333"/>
                </svg>
            </button>
            <div class="drop-header">
                {foreach item=item from=$basket.items}
                    <div class="title bask">
                        <span>{webtext keyword='basketItemTitleLabel' value='¬ ÍÓÁËÌÂ' remark='Shopping bag page'}</span>
                        <span class="topCartCount">{$productsAmount}</span>
                        <span class="plurProd">{webtext keyword="shopping-cart-product_count" value="ÚÓ‚‡ " remark=""}</span>
                        <span>{webtext keyword='basketItemTitleLabel2' value='—ÛÏÏ‡' remark='Shopping bag page'}</span>
                        <span class="topCartTotalPrice">{price_format price = $item.pricePerProduct}</span>
                        <span class="curr">{webtext keyword="shopping-cart-product_symbol" value="Û·" remark=""}</span>
                    </div>
                {/foreach}
            </div>
            <tbody>
                ...
                <tr {if $item.product.visible eq 0} class="sb-product-not-for-sale" {/if}>
                    ...
                    <td>
                        ...
                        <input type="text" onchange="ShoppingBag.changeQty({$item.id})" value="{$item.quantity}"
                               class="input sb-quantity-input sb-quantity-{$item.id}">
...
<script>
    ...
    var Basket = {
        ...
        show: function (li) {
            ...
            base.$productPhoto.attr('src', $('a img', li).attr('src'));
            base.$productTitle.html($('a .title', li).text());
...
//                base.$productColor.attr($('.product_color_check .selected_color').attr('title'));
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.success) {
                        ...
                        base.showListPopUp();
                        if(base.detailList.length < 3){
                            info.photo = $('a img', btn.offsetParent).attr('src');
                            info.title = $('a', btn.offsetParent).attr('title');
                            base.detailList.push(info);
                        }
                    }
        ...
        showListPopUp: function(){
            var html = '<div class="shopping-cart-header">' + listPopUpTitle + '</div>';
            console.log(this.detailList);
            for(var i = 0; i < this.detailList.length; ++i){
                html += '<img class="shopping-cart-product-photo" src="' + this.detailList[i].photo + '" alt="">';
                html += '<div class="shopping-cart-info">';
                html += '<div class="shopping-cart-product-title">' + this.detailList[i].title + '</div></div>';
                html += '<div class="clear"></div>';
            }
            $('#basketListPopUp .shopping-cart-content').html(html);
            this.$cartList.stop().fadeIn();
        },
...
{literal}
    <script>
        ...
        $("#tinyBask").hover(function () {
            Basket.showListPopUp();
        }, function(){
            Basket.$cartList.stop().fadeOut();
        });
    </script>
{/literal}