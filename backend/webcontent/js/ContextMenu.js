...
DHTMLGoodies_menuModel.prototype = {
    ...
    __getDepths : function()
    {
    	for(var no in this.menuItems)
    	{
    		...
    		{
    			this.menuItems[this.menuItems[no]['parentId']]['depth'] = this.menuItems[this.menuItems[no]['parentId']]['depth']+1;
    		}
    	}
    }
    ,
...
DHTMLGoodies_contextMenu = function()
{
	...
	this.imagePath = 'webcontent/img/tree/';
}
...
DHTMLGoodies_contextMenu.prototype = 
{
	...
	__parentMouseOver : function()
	{
		this.className = 'DHTMLSuite_item_mouseover';	
		child = this.getElementsByTagName('UL');
	...
	__parentMouseOut : function()
	{
		...
		//setTimeout('child[0].style.display = "none"',1000);
	}
	// }}}
	,
			...
			if(!this.menuItems[no]['parentId'])
			{
				...
				else
				{
					...
					if(this.menuItems[no]['depth']>1)
					{
						...
						for(var j in this.menuItems)
						{
							if(this.menuItems[j]['parentId'] == this.menuItems[no]['id'])
							{
								var childLi = document.createElement('LI');								
								if(this.menuItems[j]['jsFunction'])
								{
									this.menuItems[j]['url'] = this.menuItems[j]['jsFunction'] + '(this,referenceToDHTMLSuiteContextMenu.srcElement)';
								}
								if(this.menuItems[j]['itemIcon'])
								{
									childLi.style.backgroundImage = 'url(\'' + this.menuItems[j]['itemIcon'] + '\')';
									if(!document.all)
										childLi.style.backgroundPosition = '1px center';
								}
								
								childLi.innerHTML = '<a href="#" onclick="return false">' + this.menuItems[j]['itemText'] + '</a>';
								childLi.onmouseover = this.__mouseOver;
								childLi.onmouseout = this.__mouseOut;
								childUl.appendChild(childLi);
							}
						}
						childUl.className = "subMenu";
						li.onmouseover = this.__parentMouseOver;
						li.onmouseout = this.__parentMouseOut;
