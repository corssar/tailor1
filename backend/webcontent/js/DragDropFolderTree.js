...
************************************************************************************************************/	
	...
	function JSDragDropTree()
	{
		...
		this.imageFolder = 'webcontent/img/tree/';
		//this.folderImage = 'dhtmlgoodies_sheet.gif';//'dhtmlgoodies_folder.gif';
		this.folderImage = 'leaf.gif';
		...
		//properties for relate with CMS
		var itemViewId;
		var moduleViewId;
		var fieldId;
		var menuId;
	...
	JSDragDropTree.prototype = {
		...
		showHideNode : function(e,inputId)
		{
			...
			/* Next part uses for saving expanded nodes order before page refreshes. Comitet by Andrew G because we don't need this feature for iproaction backend programm
		...
		initDrag : function(e)
		{
			...
			//document.getElementById('dhtmlTreeContainer').style.cursor = 'move';
		...
		dropDragableNodes:function()
		{
			...
			/*Added by Andrew for fieldType = 29*/
			document.getElementById(JSTreeObj.fieldId+'nodeOrders').value=JSTreeObj.getNodeOrders()
		}
		,
		...
		{
			addTreeItem(obj2.parentNode.id, this.itemViewId, this.menuId);
		}
		,
		addModule : function(obj1,obj2)
		{
			addTreeItem(obj2.parentNode.id, this.moduleViewId, this.menuId);
		}
		,
		deleteItem : function(obj1,obj2)
		{
			//alert('obj2'+obj2.parentNode.id);
			/*deleteTreeItem(obj2.parentNode.id, this.itemViewId);*/
			...
			if(confirm(message)){
				...
				this.__deleteComplete(1,obj2.parentNode);
		...
		{
			if(/*this.ajaxObjects[ajaxIndex].response!='OK'*/false){
		...
		renameItem : function(obj1,obj2)
		{
			...
			editTreeItem(obj2.parentNode.id, this.itemViewId);
			//tree.editItem(1,currentItemToEdit.id);
		...
		,
		renameModule : function(obj1,obj2)
		{
			editTreeItem(obj2.parentNode.id, this.moduleViewId);
		}
		,
		initTree : function()
		{
			...
			if(this.deleteAllowed || this.renameAllowed)
			{
				try{
					...
					if(this.treeType == 'menu')
					{
						...
						if(this.deleteAllowed)	this.menuModel.addItem(1,DELETE,'webcontent/img/item_delete.gif','',false,'JSTreeObj.deleteItem');
						if(this.renameAllowed)	this.menuModel.addItem(2,EDIT,'webcontent/img/reply.gif','',false,'JSTreeObj.renameItem');
												this.menuModel.addItem(3,ADD_ITEM,'webcontent/img/action_add.gif','',false,'JSTreeObj.addItem');
												this.menuModel.addItem(4,ADD_MODULE,'webcontent/img/module.gif','',false,'JSTreeObj.addModule');
						this.menuModel.init();
						
						this.rootItemMenu = new DHTMLGoodies_menuModel();
						this.rootItemMenu.addItem(1,ADD_ITEM,'webcontent/img/action_add.gif','',false,'JSTreeObj.addItem');
						this.rootItemMenu.addItem(2,ADD_MODULE,'webcontent/img/module.gif','',false,'JSTreeObj.addModule');
						this.rootItemMenu.init();
						
						this.moduleItemMenu = new DHTMLGoodies_menuModel();
						this.moduleItemMenu.addItem(1,DELETE,'webcontent/img/item_delete.gif','',false,'JSTreeObj.deleteItem');
						this.moduleItemMenu.addItem(2,EDIT,'webcontent/img/reply.gif','',false,'JSTreeObj.renameModule');
						this.moduleItemMenu.init();
						
						this.contextMenu.setWidth(130);
					}
					else if(this.treeType == 'catalog')
					{
						this.menuModel = new DHTMLGoodies_menuModel();
						if(this.deleteAllowed)	this.menuModel.addItem(1,DELETE,'webcontent/img/item_delete.gif','',false,'JSTreeObj.deleteItem');
						if(this.renameAllowed)	this.menuModel.addItem(2,EDIT,'webcontent/img/reply.gif','',false,'JSTreeObj.renameItem');
												this.menuModel.addItem(3,ADD_SUBCATEGORY,'webcontent/img/action_add.gif','',false,'JSTreeObj.addItem');
						this.menuModel.init();
						
						this.rootItemMenu = new DHTMLGoodies_menuModel();
						this.rootItemMenu.addItem(1,ADD_CATEGORY,'webcontent/img/action_add.gif','',false,'JSTreeObj.addItem');
						this.rootItemMenu.init();
						this.contextMenu.setWidth(180);
					}
					else if(this.treeType == 'tree')
					{
						this.menuModel = new DHTMLGoodies_menuModel();
						if(this.deleteAllowed)	this.menuModel.addItem(1,DELETE,'webcontent/img/item_delete.gif','',false,'JSTreeObj.deleteItem');
						if(this.renameAllowed)	this.menuModel.addItem(2,EDIT,'webcontent/img/reply.gif','',false,'JSTreeObj.renameItem');
						...
						this.rootItemMenu = new DHTMLGoodies_menuModel();
						this.rootItemMenu.addItem(1,ADD_POI_TYPE,'webcontent/img/action_add.gif','',false,'JSTreeObj.addItem');
						this.rootItemMenu.init();
						
						this.noChildrenItemMenu = new DHTMLGoodies_menuModel();
						if(this.deleteAllowed)	this.noChildrenItemMenu.addItem(1,DELETE,'webcontent/img/item_delete.gif','',false,'JSTreeObj.deleteItem');
						if(this.renameAllowed)	this.noChildrenItemMenu.addItem(2,EDIT,'webcontent/img/reply.gif','',false,'JSTreeObj.renameItem');
						this.noChildrenItemMenu.init();
			...
			for(var no=0;no<menuItems.length;no++)
			{
				...
				var itemImage = menuItems[no].getAttribute('itemImage')?menuItems[no].getAttribute('itemImage'):null;
				
				if(itemImage)
				{
					folderImg.src = this.imageFolder + itemImage;
				}else{
					folderImg.src = this.imageFolder + this.folderImage;
				}
				...
				if(this.contextMenu)
				{
					if(menuItems[no].getAttribute('isRoot')=='true')
					{
						this.contextMenu.attachToElement(aTag,false,this.rootItemMenu);
					}
					else if(menuItems[no].getAttribute('isModule')=='true')
					{
						this.contextMenu.attachToElement(aTag,false,this.moduleItemMenu);
					}
					else if(menuItems[no].getAttribute('noChildren')=='true')
					{
						this.contextMenu.attachToElement(aTag,false,this.noChildrenItemMenu);
					}
					else
					{
						this.contextMenu.attachToElement(aTag,false,this.menuModel);						
					}
				}
				this.addEvent(aTag,'contextmenu',this.highlightItem);
				this.addEvent(aTag,'click',this.highlightItem);
			}
			/* Next part uses for saving expanded nodes order after page refreshed. Comitet by Andrew G because we don't need this feature for iproaction backend programm
		...
		,
		addTreeNode : function(id,parentNodeId)
		{
			var parentNode = document.getElementById('node'+parentNodeId);
			var parentSubItems = parentNode.getElementsByTagName('UL');
			var imgParent = parentNode.getElementsByTagName('IMG')[0];
			imgParent.style.visibility = 'visible';	// Show [+],[-] icon
			...
			aTag.id = 'nodeATag' + node.id.replace(/[^0-9]/gi,'');
			if(!noDrag)aTag.onmousedown = JSTreeObj.initDrag;
			if(!noChildren)aTag.onmousemove = JSTreeObj.moveDragableNodes;
			...
			var itemImage = node.getAttribute('itemImage')?node.getAttribute('itemImage'):null;
			...
			if(this.contextMenu)
			{
				if(node.getAttribute('isRoot')=='true')
				{
					this.contextMenu.attachToElement(aTag,false,this.rootItemMenu);						
				}
				else if(node.getAttribute('isModule')=='true')
				{
					this.contextMenu.attachToElement(aTag,false,this.moduleItemMenu);
				}
				else
